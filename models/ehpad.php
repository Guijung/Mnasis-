...
class ehpad{
  ...
  public function updateEhpad()
  {
    $updatedEhpad = $this->db->prepare('
      ...
      WHERE id = :id
    ');
    $updatedEhpad->bindValue(':id', $this->id, PDO::PARAM_INT);
    $updatedEhpad->bindValue(':email', $this->email, PDO::PARAM_STR);
    $updatedEhpad->bindValue(':name', $this->name, PDO::PARAM_STR);
    $updatedEhpad->bindValue(':city', $this->city, PDO::PARAM_STR);
    return $updatedEhpad->execute();
  }
}
?>
